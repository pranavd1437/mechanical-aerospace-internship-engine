...
class TestScopedHeader:
    def test_mechanical_fork_copy_is_scope_aware(self):
        cfg = {
        ...
        text = "\n".join(readme._header(
            cfg, total_open=10, companies=4000, new_week=3, shown=10,
        ))
        assert "Summer 2027 Mechanical & Aerospace Internships" in text
        assert "design/CAD" in text
        assert "subscribe by email" not in text
