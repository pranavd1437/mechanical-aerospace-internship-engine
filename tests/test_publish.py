...
class TestFeed:
    ...
    def test_feed_title_uses_configured_scope(self, monkeypatch, tmp_path):
        _redirect(monkeypatch, tmp_path)
        monkeypatch.setattr(
            publish.config,
            "load_config",
            lambda: {"cycles": ["Summer 2027"], "role_scope": "mechanical"},
        )
        publish.write_feed(STORE)
        root = ET.parse(tmp_path / "feed.xml").getroot()
        title = root.find("{http://www.w3.org/2005/Atom}title").text
        assert title == "Summer 2027 Mechanical & Aerospace Internships — new roles"
