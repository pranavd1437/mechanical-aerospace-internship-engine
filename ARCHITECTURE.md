...
[![CI](https://github.com/pranavd1437/mechanical-aerospace-internship-engine/actions/workflows/ci.yml/badge.svg)](https://github.com/pranavd1437/mechanical-aerospace-internship-engine/actions/workflows/ci.yml)
...
| `out-of-scope` | It's still posted, but fails our filters (wrong country, off-cycle, or wrong role scope) | none — our own verdict |
...
| `src/intern_engine/filters.py` | Classification: internship? configured role scope? season/year? US/Canada? category. |
...
| `.github/workflows/update.yml` | Scheduled CI (every six hours): run update, verify, commit, and push refreshed public data. |
...
- `role_scope` — `"mechanical"` keeps mechanical/aerospace and adjacent physical-
  engineering roles; `"tech"` preserves the upstream software/data/ML/security
  filter; `"all"` keeps every internship. Unknown values stop the run instead
  of silently widening the list.
