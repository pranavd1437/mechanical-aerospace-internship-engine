...
def generate(store_data: dict, stats: dict) -> None:
    ...
    scope_search_example = config.scope_search_example(cfg)
...
<title>{escape(scope_title)} Internship Engine - Live Dashboard</title>
<meta name="description" content="{escape(cycles_phrase)} {escape(scope_name)} internships, refreshed every six hours. Auto-detected citizenship and visa restrictions, salary, and skill tags.">
<meta property="og:title" content="{escape(scope_title)} Internship Engine - Live Dashboard">
<meta property="og:description" content="{len(open_jobs)} open {escape(scope_name)} internships, refreshed every six hours with source-first application links.">
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  <header class="top">
    <h1>{escape(region)} {escape(scope_name)} internships</h1>
    <p class="sub">Read straight from {stats.get('companies_total', 0):,} employer
    job boards, rebuilt every six hours. Updated {escape(updated)}.</p>
  ...
  <div class="filters" id="filters">
    <input id="q" type="search" placeholder="Search company, role, location, or skill (try “{escape(scope_search_example)}”)…" autocomplete="off">
...
def _write_unsubscribe(cfg: dict) -> None:
    ...
    endpoint = config.signup_endpoint(cfg)
    if not endpoint:
        # A fork may deliberately remove the upstream signup endpoint. Do not
        # leave an older generated page behind with somebody else's database
        # URL embedded in it.
        stale = os.path.join(paths.DOCS_DIR, "unsubscribe.html")
        try:
            os.remove(stale)
